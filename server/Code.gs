  ...
  try {
    ...
    switch(action) {
      ...
      case 'getPendingRequests':
        result = getPendingRequests(data);
        break;
      case 'getRequestStats':
        result = getRequestStats(data);
        break;
      case 'getMostWantedCourses':
        result = getMostWantedCourses(data);
        break;
      case 'getTotalRequestsReport':
        result = getTotalRequestsReport(data);
        break;
      default:
...
/**
 * Get pending requests grouped by approver role
 * Returns:
 * - Pending requests for each advisor
 * - Pending requests for each HOD
 * - Pending requests for each registrar
 */
function getPendingRequests(data) {
  ...
  // Create maps for advisor, HOD, and registrar names
  const advisorMap = {};
  for (let i = 1; i < advisorsData.length; i++) {
    advisorMap[advisorsData[i][0]] = advisorsData[i][2]; // Name is in column C
  }
  
  const hodMap = {};
  for (let i = 1; i < hodsData.length; i++) {
    hodMap[hodsData[i][0]] = hodsData[i][2]; // Name is in column C
  }
  
  const registrarMap = {};
  for (let i = 1; i < registrarsData.length; i++) {
    registrarMap[registrarsData[i][0]] = registrarsData[i][2]; // Name is in column C
  }
  
  // Count pending requests
  const advisorPending = {};
  const hodPending = {};
  const registrarPending = {};
  
  for (let i = 1; i < requestsData.length; i++) {
    const row = requestsData[i];
    const status = row[10]; // Column K - status
    const advisorId = row[17]; // Column R - advisorId
    
    // Count by advisor (awaiting_advisor status)
    if (status === 'awaiting_advisor' && advisorId) {
      const advisorName = advisorMap[advisorId] || advisorId;
      advisorPending[advisorName] = (advisorPending[advisorName] || 0) + 1;
    }
    
    // Count by HOD (awaiting_hod status)
    if (status === 'awaiting_hod') {
      const advisorName = row[18]; // Column S - advisorName (approver info)
      // We need to get HOD info from student's department - use a more intelligent approach
      // For now, we'll count all awaiting_hod requests
      hodPending['ALL_HODS'] = (hodPending['ALL_HODS'] || 0) + 1;
    }
    
    // Count by registrar (awaiting_registrar status)
    if (status === 'awaiting_registrar') {
      registrarPending['ALL_REGISTRARS'] = (registrarPending['ALL_REGISTRARS'] || 0) + 1;
    }
  }
  
  return {
    pendingByAdvisor: advisorPending,
    pendingByHOD: hodPending,
    pendingByRegistrar: registrarPending,
    summary: {
      totalAdvisorPending: Object.values(advisorPending).reduce((a, b) => a + b, 0),
      totalHODPending: Object.values(hodPending).reduce((a, b) => a + b, 0),
      totalRegistrarPending: Object.values(registrarPending).reduce((a, b) => a + b, 0)
    }
  };
}

/**
 * Get request statistics for a given date or date range
 * Returns:
 * - Requests approved in a day
 * - Requests applied (submitted) in a day
 * - Requests rejected in a day
 */
function getRequestStats(data) {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const requestsSheet = ss.getSheetByName(SHEETS.REQUESTS);
  const requestsData = requestsSheet.getDataRange().getValues();
  
  // If date range not provided, use today
  ...
  for (let i = 1; i < requestsData.length; i++) {
    const row = requestsData[i];
    const submittedDate = row[11]; // Column L - submittedDate
    const status = row[10]; // Column K - status
    
    // Extract date part (YYYY-MM-DD)
    let rowDateStr = '';
    if (submittedDate instanceof Date) {
      rowDateStr = submittedDate.toISOString().split('T')[0];
    } else if (typeof submittedDate === 'string') {
      rowDateStr = submittedDate.split('T')[0];
    }
...
/**
 * Get most wanted courses - courses that students requested the most
 * Returns top N most requested courses
 */
function getMostWantedCourses(data) {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const requestsSheet = ss.getSheetByName(SHEETS.REQUESTS);
  const requestsData = requestsSheet.getDataRange().getValues();
  ...
  for (let i = 1; i < requestsData.length; i++) {
    const row = requestsData[i];
    const courseCode = row[5]; // Column F - courseCode
    const courseName = row[6]; // Column G - courseName
    const requestType = row[4]; // Column E - requestType (add/drop)
    
    // Only count 'add' requests for most wanted courses
    if (requestType === 'add' && courseCode) {
  ...
  const sortedCourses = Object.entries(courseCount)
    ...
    .map(([code, count]) => ({
      courseCode: code,
      courseName: courseDetails[code].name,
      requestCount: count
    }));
  
  return {
    mostWantedCourses: sortedCourses,
    totalUniqueCourses: Object.keys(courseCount).length
  };
}

/**
 * Get total requests report with detailed information
 * Returns all requests with comprehensive details grouped by status
 */
function getTotalRequestsReport(data) {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const requestsSheet = ss.getSheetByName(SHEETS.REQUESTS);
  const requestsData = requestsSheet.getDataRange().getValues();
  const studentsSheet = ss.getSheetByName(SHEETS.STUDENTS);
  ...
  for (let i = 1; i < studentsData.length; i++) {
    studentMap[studentsData[i][0]] = {
      universityId: studentsData[i][2] || '',
      level: studentsData[i][6] || '',
      department: studentsData[i][5] || '',
      gpa: studentsData[i][10] || ''
    };
  }
  ...
  for (let i = 1; i < requestsData.length; i++) {
    ...
    requests.push({
      ...
      studentName: row[2],
      studentEmail: row[3],
      universityId: studentInfo.universityId || 'N/A',
